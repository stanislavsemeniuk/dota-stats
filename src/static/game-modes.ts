function findGameModeInfo(game_mode: number) {
  return gameModesStaticData.find((el) => el.id === game_mode)?.name;
}
...
export function getGameModeName(game_modeId: number) {
  return localizeGameModeName(extractGameModeName(findGameModeInfo(game_modeId) || ''));
}

const gameModesStaticData = [
  {
    id: 0,
